  ...
  return (
    <div className="score-display" data-testid="score-display">
      {stats.map((stat) => {
        switch (stat) {
          case "score":
            return <StatRow key={stat} label="SCORE" value={scoring.score.toLocaleString()} testId="stat-score" />;
          case "level":
            return <StatRow key={stat} label="LEVEL" value={scoring.level} testId="stat-level" />;
          case "lines":
            return <StatRow key={stat} label="LINES" value={scoring.lines} testId="stat-lines" />;
          case "timer":
            if (modeConfig.goal === "time" && modeConfig.goalValue != null) {
              // Ultra: count down
              return <StatRow key={stat} label="TIME" value={formatTime(modeConfig.goalValue - elapsedMs)} testId="stat-timer" />;
            }
            // Sprint: count up
            return <StatRow key={stat} label="TIME" value={formatTime(elapsedMs)} testId="stat-timer" />;
          case "linesRemaining":
            return (
              <StatRow
                key={stat}
