/**
 * Reference verification tests — classic scoring and gravity tables.
 *
 * These are table-driven snapshot tests that hardcode the authoritative
 * reference values from the NTSC classic specification. Any accidental
...
import { classicDropInterval } from "../engine/gravity.js";
...
/**
 * NTSC classic gravity table: frames per gridcell drop at each level.
...
/** NTSC classic frame rate used to convert frames → milliseconds. */
...
/**
 * classic level progression: first level-up line thresholds.
...
describe("classic reference: gravity table", () => {
  it.each(REFERENCE_GRAVITY)(
    "level $level → $frames frames",
    ({ level, frames }) => {
      const expectedMs = Math.round((frames / NES_FPS) * 1000);
      expect(classicDropInterval(level)).toBe(expectedMs);
    },
  );

  it("levels beyond 29 clamp to 1 frame (kill screen)", () => {
    const killScreenMs = Math.round((1 / NES_FPS) * 1000);
    expect(classicDropInterval(30)).toBe(killScreenMs);
    expect(classicDropInterval(50)).toBe(killScreenMs);
    expect(classicDropInterval(99)).toBe(killScreenMs);
  });

  it("complete table snapshot", () => {
    const actual = REFERENCE_GRAVITY.map(({ level }) => classicDropInterval(level));
...
describe("classic reference: scoring table", () => {
  ...
  describe("base points × (level + 1)", () => {
    for (const { lines, label, basePoints } of REFERENCE_LINE_POINTS) {
      ...
      it.each(testLevels)(
        ...
        (level) => {
          const state = ClassicScoring.createState(level);
          ClassicScoring.onLineClear(state, lines, "none", false);
  ...
  it("soft drop awards 1 point per cell, independent of level", () => {
    for (const level of testLevels) {
      const state = ClassicScoring.createState(level);
      ClassicScoring.onSoftDrop(state, 10);
      expect(state.score).toBe(10);
    }
  });

  it("hard drop is a no-op (not in original classic)", () => {
    const state = ClassicScoring.createState(0);
    ClassicScoring.onHardDrop(state, 20);
    expect(state.score).toBe(0);
  });

  describe("classic ignores non-classic features", () => {
    it("T-spin type has no effect on score", () => {
      const normal = ClassicScoring.createState(0);
      ClassicScoring.onLineClear(normal, 1, "none", false);

      const full = ClassicScoring.createState(0);
      ClassicScoring.onLineClear(full, 1, "full", false);

      const mini = ClassicScoring.createState(0);
      ClassicScoring.onLineClear(mini, 1, "mini", false);
    ...
    it("perfect clear flag has no effect on score", () => {
      const normal = ClassicScoring.createState(0);
      ClassicScoring.onLineClear(normal, 4, "none", false);

      const pc = ClassicScoring.createState(0);
      ClassicScoring.onLineClear(pc, 4, "none", true);
...
describe("classic reference: level progression", () => {
  describe("first level-up thresholds", () => {
    it.each(REFERENCE_LEVEL_THRESHOLDS)(
      ...
      ({ startLevel, threshold }) => {
        const state = ClassicScoring.createState(startLevel);
  ...
  it("start level beyond gravity table still uses correct scoring multiplier", () => {
    const state = ClassicScoring.createState(30);
    ClassicScoring.onLineClear(state, 4, "none", false);
